...
table <- MIR_data$sample_data %>%
  ...
  # Set factor levels to preserve desired order
  mutate(PROT = factor(PROT, levels = c("Outside", "Inside"))) %>%
  ...
  arrange(PROT, STRAT) %>%  # ensure Outside comes first, then Inside
  ungroup()

# Conditional rendering for HTML and PDF
if (knitr::is_html_output()) {
  DT::datatable(
    table,
    class = "cell-border stripe", 
    rownames = FALSE,
    colnames = c("Study Area", "Strata Name", "Strata Description", sort(unique(MIR_data$sample_data$YEAR))),
    caption = "Table 1. Number of sites sampled by year.",
    options = list(
      columnDefs = list(list(className = 'dt-center', targets = "_all")),
      info = FALSE,
      paging = FALSE,
...
spp_table <- spp_list %>%
  select(SPECIES_CD, COMNAME, SCINAME) %>%
  mutate(COMNAME = sub("^([a-z])", "\\U\\1", COMNAME, perl = TRUE))  # capitalize first letter
...
pmap(spp_list, function(SPECIES_CD, COMNAME, SCINAME, max_size) {
